...
export default function Search() {
  ...
  useEffect(() => {
    ...
    // If we have an ID but no movie object, fetch the movie
    if (movieId) {
      const fetchSingleMovie = async () => {
        setLoading(true);
        setError(null);
        try {
          const results = await fetchAutocompleteMovies(movieId);
          const movie = results.find((m: { id: string; }) => m.id === movieId);
